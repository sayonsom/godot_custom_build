...
const NATIVE_BRIDGE_SINGLETON := "FloorPlannerBridge"
...
func _ready() -> void:
	...
	_bind_native_bridge()
	_emit_bridge_event("engine_ready", _build_runtime_state("engine_ready"))
...
func _bind_native_bridge() -> void:
	if !Engine.has_singleton(NATIVE_BRIDGE_SINGLETON):
		return

	_native_bridge = Engine.get_singleton(NATIVE_BRIDGE_SINGLETON)
	if _native_bridge == null or !_native_bridge.has_signal("command_requested"):
		return

	var handler := Callable(self, "_on_bridge_command_requested")
	if !_native_bridge.is_connected("command_requested", handler):
		_native_bridge.connect("command_requested", handler)

func _on_bridge_command_requested(command: String, payload_json: String) -> void:
	var payload := _parse_command_payload(payload_json)
	var result := _execute_bridge_command(command, payload)
	var response := {
		"command": command,
		"ok": result["ok"],
		"message": result["message"],
	}
	_emit_bridge_event("command_result", response)
	if result["ok"] and result["emit_state"]:
		_emit_bridge_event("state", _build_runtime_state("command:%s" % command))
	elif !result["ok"]:
		_emit_bridge_event("error", response)

func _parse_command_payload(payload_json: String) -> Dictionary:
...
func _execute_bridge_command(command: String, payload: Dictionary) -> Dictionary:
	match command:
		"request_state":
			return {
				"ok": true,
				"message": "state_requested",
				"emit_state": true,
			}
		"switch_floor":
			var floor_index := -1
			if payload.has("index"):
				floor_index = int(payload["index"])
			elif payload.has("id"):
				floor_index = _find_floor_index_by_id(String(payload["id"]))

			if floor_index < 0 or floor_index >= _floor_states.size():
				return {
					"ok": false,
					"message": "switch_floor requires a valid index or id",
					"emit_state": false,
				}

			_set_active_floor(floor_index)
			return {
				"ok": true,
				"message": "floor_switched",
				"emit_state": true,
			}
		"set_plan_edit":
			if !payload.has("enabled"):
				return {
					"ok": false,
					"message": "set_plan_edit requires enabled",
					"emit_state": false,
				}

			_set_plan_edit_mode(bool(payload["enabled"]))
			return {
				"ok": true,
				"message": "plan_edit_updated",
				"emit_state": true,
			}
		"add_room":
			if !_plan_edit_mode or _current_floor_index < 0:
				return {
					"ok": false,
					"message": "add_room requires plan edit mode on an active floor",
					"emit_state": false,
				}
			_add_room_to_current_floor()
			return {
				"ok": true,
				"message": "room_added",
				"emit_state": true,
		...
		"rotate_selected":
			if placement_controller.get_selected_target() == null:
				return {
					"ok": false,
					"message": "rotate_selected requires an active selection",
					"emit_state": false,
				}
			placement_controller.rotate_selected(float(payload.get("delta_degrees", 15.0)))
			return {
				"ok": true,
				"message": "selection_rotated",
				"emit_state": true,
			}
		"flip_selected_x":
			if placement_controller.get_selected_target() == null:
				return {
					"ok": false,
					"message": "flip_selected_x requires an active selection",
					"emit_state": false,
				}
			placement_controller.flip_selected_x()
			return {
				"ok": true,
				"message": "selection_flipped_x",
				"emit_state": true,
			}
		"flip_selected_z":
			if placement_controller.get_selected_target() == null:
				return {
					"ok": false,
					"message": "flip_selected_z requires an active selection",
					"emit_state": false,
				}
			placement_controller.flip_selected_z()
			return {
				"ok": true,
				"message": "selection_flipped_z",
				"emit_state": true,
			}
		"clear_selection":
		...
		"delete_selected":
			if placement_controller.get_selected_target() == null:
				return {
					"ok": false,
					"message": "delete_selected requires an active selection",
					"emit_state": false,
				}
			_delete_selected()
			return {
				"ok": true,
				"message": "selection_deleted",
				"emit_state": true,
			}
		"place_asset":
			if !payload.has("asset_id"):
				return {
					"ok": false,
					"message": "place_asset requires asset_id",
					"emit_state": false,
				}

			var asset_id := String(payload["asset_id"]).strip_edges()
			if !ASSET_CATALOG.has(asset_id):
				return {
					"ok": false,
					"message": "Unknown asset_id: %s" % asset_id,
					"emit_state": false,
				}

			var floor_index := _current_floor_index
			if payload.has("floor_index"):
				floor_index = int(payload["floor_index"])
			elif payload.has("floor_id"):
				floor_index = _find_floor_index_by_id(String(payload["floor_id"]))

			if floor_index < 0 or floor_index >= _floor_states.size():
				return {
					"ok": false,
					"message": "place_asset requires a valid floor index/id",
					"emit_state": false,
				}

			if !payload.has("x") or !payload.has("z"):
				return {
					"ok": false,
					"message": "place_asset requires x and z",
					"emit_state": false,
				}

			var world_position := Vector3(float(payload["x"]), FLOOR_HEIGHT * 0.5, float(payload["z"]))
			world_position = _clamp_to_floor(world_position, floor_index)
			var rotation_override: Variant = null
			if payload.has("rotation"):
				rotation_override = float(payload["rotation"])
			var wrapper := _spawn_asset_on_floor(asset_id, world_position, floor_index, rotation_override)
			if wrapper == null:
				return {
					"ok": false,
					"message": "asset spawn failed for %s" % asset_id,
					"emit_state": false,
				}

			if _current_floor_index != floor_index:
				_set_active_floor(floor_index)
			placement_controller.select_target(wrapper)
			return {
				"ok": true,
				"message": "asset_placed",
				"emit_state": true,
			}
		"set_outline_thickness":
			if !payload.has("value"):
				return {
					"ok": false,
					"message": "set_outline_thickness requires value",
					"emit_state": false,
				}
			placement_controller.set_outline_thickness(float(payload["value"]))
			return {
				"ok": true,
				"message": "outline_thickness_updated",
				"emit_state": true,
			}
		"reset_camera":
			if camera_3d.has_method("reset_view"):
				camera_3d.reset_view()
			return {
				"ok": true,
				"message": "camera_reset",
				"emit_state": true,
			}
		_:
			return {
...
func _find_floor_index_by_id(floor_id: String) -> int:
	for index in range(FLOOR_PLANS.size()):
		if String(FLOOR_PLANS[index]["id"]) == floor_id:
			return index
	return -1

func _build_runtime_state(reason: String = "") -> Dictionary:
	var floors: Array = []
	...
	var state := {
		"reason": reason,
		"current_floor_index": _current_floor_index,
		"current_floor_id": "",
		"current_floor_name": "",
		"plan_edit_mode": _plan_edit_mode,
		"selected_room_index": _selected_room_index,
		"selected_target": null,
		"floors": floors,
	}

	if _current_floor_index >= 0 and _current_floor_index < FLOOR_PLANS.size():
		var floor_data: Dictionary = FLOOR_PLANS[_current_floor_index]
		state["current_floor_id"] = floor_data["id"]
		state["current_floor_name"] = floor_data["name"]

	var selected_target := placement_controller.get_selected_target()
	if selected_target != null:
		state["selected_target"] = {
			"name": selected_target.name,
			"position": {
				"x": selected_target.position.x,
				"y": selected_target.position.y,
				"z": selected_target.position.z,
			},
			"rotation_degrees_y": selected_target.rotation_degrees.y,
			"scale": {
				"x": selected_target.scale.x,
				"y": selected_target.scale.y,
				"z": selected_target.scale.z,
			},
		}

	return state

func _emit_bridge_event(event: String, payload: Dictionary) -> void:
	if _native_bridge == null or !_native_bridge.has_method("onGodotEvent"):
		return
	_native_bridge.onGodotEvent(event, JSON.stringify(payload))
...
func _select_room(room_index: int) -> void:
	...
	_emit_bridge_event("state", _build_runtime_state("room_selected"))
...
func _clear_room_selection() -> void:
	...
	_refresh_active_floor_room_visuals()
	_update_plan_edit_ui()
	_emit_bridge_event("state", _build_runtime_state("room_selection_cleared"))
...
func _end_room_drag() -> void:
	...
	_emit_bridge_event("state", _build_runtime_state("room_drag_finished"))
...
func _set_plan_edit_mode(enabled: bool) -> void:
	...
	_update_plan_edit_ui()
	_emit_bridge_event("state", _build_runtime_state("plan_edit_mode"))
...
func _add_room_to_current_floor() -> void:
	...
	_emit_bridge_event("state", _build_runtime_state("room_added"))
...
func _set_active_floor(index: int) -> void:
	...
	_update_plan_edit_ui()
	_emit_bridge_event("state", _build_runtime_state("floor_switched"))
...
func _update_selection_ui(target: Node3D) -> void:
	if _plan_edit_mode:
		_update_plan_edit_ui()
		_emit_bridge_event("state", _build_runtime_state("selection_ignored_plan_edit"))
		return
...
func _on_target_transform_changed(target: Node3D) -> void:
	if target != null:
		...
		_emit_bridge_event("target_transform", {
			...
			"rotation_degrees_y": target.rotation_degrees.y,
		})
